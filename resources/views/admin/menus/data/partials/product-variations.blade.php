        ...
        <div class="collapse" id="collapseVariation{{ $identity ?? $productVariation->id ?? '' }}">
            ...
            <div class="row g-0 p-3 bg-light">
                ...
                <div class="col-12 col-md-6">
                    <div class="mb-3">
                        <label for="">{{ __('Giá bán lẻ') . ' (' . config('custom.currency') . ')' }}</label>
                        <x-input id="inputPVP{{ $identity ?? $productVariation->id ?? '' }}" 
                            name="products_variations[price_retail][{{$identity ?? $productVariation->id ?? ''}}]" 
                            :value="$productVariation->price_retail ?? ''" 
                    ...
                    <div class="">
                        <label for="mb-3">{{ __('Giá bán sỉ') . ' (' . config('custom.currency') . ')' }}</label>
                        <x-input name="products_variations[price_wholesale][{{$identity ?? $productVariation->id ?? ''}}]" 
                            :value="$productVariation->price_wholesale ?? ''" 
                            ...
                            {{-- data-parsley-lt-message="Giá bán sỉ phải nhỏ hơn giá mặc định."  --}}
                        />
                    </div>
                </div>
            </div>
